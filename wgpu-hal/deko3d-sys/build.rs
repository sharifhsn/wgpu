...
fn main() {
    ...
    let debug = env::var_os("CARGO_FEATURE_DEBUG_DEKO3D").is_some();
    let release = env::var_os("CARGO_FEATURE_RELEASE_DEKO3D").is_some();
    let deko_lib = match (debug, release) {
        (false, true) => "deko3d",
        (true, false) => "deko3dd",
        (false, false) => panic!("enable exactly one of debug-deko3d or release-deko3d"),
        (true, true) => panic!("enable exactly one of debug-deko3d or release-deko3d"),
    };
